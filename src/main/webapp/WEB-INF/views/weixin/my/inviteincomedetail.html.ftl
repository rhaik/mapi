	...
	<#list userIncomeDetail as u>
		<li class="ui-border-b">
			<div class="ui-list-info">
				<p class="ui-li-title">试玩《${u.userIncomeLog.remarks!""}》成功</p>
...
$(function(){
	...
	$(document).bind("pageinit", function() {
		isInit=0;
		serverURL = "${util.ctx}<#if fromSafari??>/ios/wxapi<#else>/weixin/api</#if>/inviteincomelogs?from_user=${from_user!0}"; //服务器地址
		startNum = 0;//当前页
		...
		callbackReviceData = function(datas){
		    ...
		    for (var i = 0; i < datas.data.length; i++) {
		        result += '<li class="ui-border-b">'
						+'<div class="ui-list-info">'
						+'	<p class="ui-li-title">试玩《'+ datas.data[i].taskName + '》成功</p>'
