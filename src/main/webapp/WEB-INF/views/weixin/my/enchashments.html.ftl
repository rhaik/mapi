...
$(function(){
 	userEnchashmentDetail = function(id){
 		var url = '${util.ctx}<#if fromSafari??>/ios<#else>/weixin</#if>/my/enchashments/' + id;
	...
	$(document).bind("pageinit", function() {
		isInit=0;
		serverURL = "${util.ctx}<#if fromSafari??>/ios/wxapi<#else>/weixin/api</#if>/enchashment/list"; //服务器地址
		startNum = 0;//当前页
